...
class PopupController extends Controller
{
    ...
    {
        $this->popup = new \App\Models\Popup;
        $this->campos = [
    ...
    {
        $tipo_atual = DB::table('cms.popups')->where('id', $id)->first();
        $status = $tipo_atual->status == 0 ? 1 : 0;
        DB::table('cms.popups')->where('id', $id)->update(['status' => $status]);
    ...
    public function positionUp($id)
    {
        ...
        $posicao_atual = DB::table('cms.popups')->where('id', $id)->first();
        $upPosicao = $posicao_atual->posicao-1;
        $posicao = $posicao_atual->posicao;

        //Coloca com a posicao do anterior
        DB::table('cms.popups')->where('posicao', $upPosicao)->update(['posicao' => $posicao]);

        //atualiza a posicao para o anterior
        DB::table('cms.popups')->where('id', $id)->update(['posicao' => $upPosicao]);
