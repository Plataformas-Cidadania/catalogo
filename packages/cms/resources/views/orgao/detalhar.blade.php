...
@section('content')
    {!! Html::script(config('app.url').'assets-cms/js/controllers/alterarOrgaoCtrl.js') !!}
    <div ng-controller="alterarOrgaoCtrl">
        <div class="box-padrao">
            <h1><a href="cms/orgaos"><i class="fa fa-arrow-circle-left"></i></a>&nbsp;&nbsp;Orgao</h1>
            <?php //print_r($orgao);?>
            <div ng-init="carregaImagem('{{$orgao->imagem}}', '{{$orgao->arquivo}}'); detalhar({{$orgao->id}})">
                <span class="texto-obrigatorio">* campos obrigatórios</span><br><br>
                {!! Form::model($orgao, ['name' =>'form']) !!}
                ...
                <a href="arquivos/orgaos/<% arquivoBD %>" target="_blank" ng-show="arquivoBD"><% arquivoBD %></a>
                <a ng-show="fileArquivo"><% fileArquivo.name %></a>
                {{--<br><br>--}}

                <br><br>
                @include('cms::orgao._form')
                <input type="hidden" name="id" ng-model="id" ng-init="id='{{$orgao->id}}'"/>
                <div class="row">
                    <div class="col-md-1 col-lg-1 col-xs-3">
                        <br>
                        <button class="btn btn-info" type="button" ng-click="alterar(picFile, fileArquivo)" ng-disabled="form.$invalid && form.orgao.$dirty">Salvar</button>
                    </div>
