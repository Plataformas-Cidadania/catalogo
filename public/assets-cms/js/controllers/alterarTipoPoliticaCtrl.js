cmsApp.controller('alterarTipoPoliticaCtrl', ['$scope', '$http', 'Upload', '$timeout', function($scope, $http, Upload, $timeout){
    ...
    $scope.detalhar = function(id){
        ...
        $http({
            ...
            $scope.tipoPolitica = data;//data.data
            $scope.processandoDetalhar = false;
        }).error(function(data){
    ...
    $scope.alterar = function (file){
        ...
        if(file==null){
            ...
            $http.put("api/tipo_politica/"+$scope.id, $scope.tipoPolitica).success(function (data){
                //console.log(data);
                $scope.processandoSalvar = false;
    ...
    $scope.carregaImagem  = function(img) {
        if(img!=''){
            $scope.imagemBD = 'imagens/tipo-politicas/xs-'+img;
