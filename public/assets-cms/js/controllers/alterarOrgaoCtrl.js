cmsApp.controller('alterarOrgaoCtrl', ['$scope', '$http', 'Upload', '$timeout', function($scope, $http, Upload, $timeout){
    ...
    $scope.alterar = function (file){
        ...
        if(file==null){
            ...
            $http.put("api/orgao/"+$scope.id, $scope.orgao).success(function (data){
                //console.log(data);
                $scope.processandoSalvar = false;
    ...
    $scope.carregaImagem  = function(img) {
        if(img!=''){
            $scope.imagemBD = 'imagens/orgaos/xs-'+img;
