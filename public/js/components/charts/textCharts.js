class TextCharts extends React.Component {
  ...
  componentDidMount() {//this.loadChart();
  }

  componentWillReceiveProps(props) {
    let areas = ['area_atuacao', 'Área de Atuação', 'pie']; //let areas = ['area_atuacao', 'evolucao_quantidade_osc_ano', 'natureza_juridica', 'orcamento', 'repasse_recursos', 'trabalhadores'];
...
>>>>>>> df028c263e547e62ae2bab713bbbd401bb6e1942
        ...
        labels.push(dataChart[j].label); //labels.push("")
  ...
  generateTable(data) {
    ...
    for (let chart in data) {
      ...
      for (let j in dataTable) {
        let table = dataTable[j]; //Quando tiver o key///////////////////////////////
  ...
  callModal(chart) {
    ...
    let table = this.state.tables[chart]; //console.log(table);
